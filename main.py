...
async def run_task(task: str = Query(...)):
    ...
    try:
        if task_code == "A1":
            user_email = "23f2000043@ds.study.iitm.ac.in"
...
def handle_task_A8():
    ...
    try:
        ...
        if passes_luhn(recognized_16):
            ...
            # If fourth digit is '8', try flipping it to '6'
            if recognized_16[3] == '8':
                possible_fix = recognized_16[0:3]+'6'+recognized_16[4:]
                if passes_luhn(possible_fix):
                    final_number = possible_fix
                else:
                    return {
                        "error": "Luhn check failed, flipping '8'->'6' also failed.",
